...
def submit(result_json: str | Path, commit: bool = False) -> int:
    command = [sys.executable, str(SKILL_QUERY_SCRIPT), "submit-result", str(result_json)]
    if commit:
...
def main() -> None:
    ...
    raise SystemExit(submit(args.result_json, args.commit))
